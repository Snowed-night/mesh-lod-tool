...
- Web 工具支持无材质预览、PBR 材质、光源调节、接触阴影和误差热力图
...
`result/web/models/`、`result/web/uploads/`、`result/web/temp/` 和 `result/web/jobs.json` 属于运行时生成内容，默认不纳入版本控制。
...
Web 后端会把上传文件复制或转换成 `result/web/models/<job>/source.obj`，再调用 C++ 工具生成 LOD。同一个模型重复上传时，后端会根据文件哈希复用已有结果，不再重复生成一套时间戳目录。
...
当前 Web 版支持 OBJ 和基础 3MF 上传。3MF 会先在后端转换为 OBJ，再进入 LOD 管线。上传后的源 OBJ 会自动保存到本次任务目录，例如 `result/web/models/<job>/source.obj`。再次上传完全相同的文件时，后端会复用 `jobs.json` 中记录的结果。
...
- 无材质：使用普通漫反射材质展示模型，方便观察光源方向、颜色和强度
- 真实材质：使用统一 PBR 展示材质、环境光和接触阴影展示模型；当前不解析 OBJ 的 MTL 贴图
...
这不是在浏览器里重写简化算法，而是前端负责交互，后端复用 C++ 简化核心。
