...
从文献和工程实践角度看，本项目采用的是“经典算法理解 + 工程库落地 + 误差驱动展示”的路线。QEM 适合解释网格简化的核心误差思想，meshoptimizer 适合处理真实复杂模型，屏幕空间误差适合把几何误差转化为可观察的像素误差，热力图则用于提升结果的可解释性。

课程项目的重点不是证明自己实现的简化器超过成熟库，而是在有限周期内把算法、指标、渲染和交互组织成一个完整系统。这样既能体现图形学算法基础，也能体现面向真实模型的工程能力。
...
- 通过多模型截图、点光源展示和自适应 LOD 对比，把算法结果转化为可用于报告和答辩的可视化证据。
